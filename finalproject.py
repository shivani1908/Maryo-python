...
class dragon:
    global firerect, Canvas, imagerect
    up = False
    down = True
    velocity = 15
...
class flames:
    ...
    def __init__(self):
        ...
        self.imagerect = pygame.Rect(window_width - 100, self.height, 20, 20)
...
class maryo:
    global moveup,movedown,cactusrect,firerect,gravity
    speed = 10
    downspeed = 20
    ...
    def update(self):
        ...
        if (gravity and (self.imagerect.bottom < firerect.top)):  #no key is pressed so score is not changed
            self.imagerect.bottom += self.speed

def terminate(): #end the game
    pygame.quit()
    sys.exit()

def waitforkey(): #wait for key to be pressed
    while True:
...
def check_level(score):  #to check the level
           global window_height,cactusrect,firerect,level
...
caption = pygame.display.set_caption("MARYO")

font = pygame.font.SysFont(None,48)
scorefont = pygame.font.SysFont(None,45)
...
while True:
    flame_list = [] #create a flame list
    player = maryo()
    ...
    while True: #begin game loop
        ...
        drawtext("Score : %s | Top Score : %s | Level : %s"%(player.score,topscore,level),scorefont,Canvas,300,cactusrect.bottom + 10)
